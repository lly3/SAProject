...
@section('content')
    <h1 class="text-xl mx-4 mt-6 my-3 flex flex-col sm:flex-row sm:text-2xl">
        @if($tag->posts->count() != 0 && $tag->posts->first()->organization->name == $tag->name)หน่วยงานที่รับผิดชอบ: @elseหัวข้อที่เกี่ยวข้อง: @endif {{ $tag->name }}
        <span class="sm:ml-auto ml-0"> จำนวน: {{ $tag->posts->count() }} คำร้อง</span>
    </h1>
