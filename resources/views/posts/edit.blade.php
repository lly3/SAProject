...
@section('content')
    ...
    <div class="min-h-screen p-0 sm:p-6">
        <div class="mx-auto w-full px-6 py-12 bg-white border-0 shadow-lg sm:rounded-3xl">
            <form id="form" action="{{ route('posts.update', ['post' => $post->id]) }}" method="POST" enctype="multipart/form-data">
                @method("PUT")
                {{ csrf_field() }}
                    ...
                    <div class='leftside md:w-1/2 w-full'>
                        ...
                        <div>
                            <div class="flex justify-end">
                                <a href="{{ route('posts.delete.images', ['post' => $post->id]) }}" class="mr-auto transition duration-200 ease-in-out bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">ลบรูปภาพทั้งหมด</a>
                            </div>
                            <div class="my-3">
                                <p class="text-red-500">***หากต้องการแก้ไขรูปภาพ ให้เลือกชุดรูปภาพใหม่ที่ต้องการแก้ไข เพราะการแก้ไขรูปภาพจะไปลบรูปภาพเก่าทั้งหมด***</p>
                                <p class="text-red-500">***หากต้องการลบรูปภาพทั้งหมด ให้กดปุ่มลบรูปภาพทั้งหมด***</p>
                            </div>
                        </div>
                    </div>
                    <div class='rightside'>
                        <h1 class="text-2xl font-bold mb-8">แก้ไขคำร้องเรียน</h1>
                            ...
                            <div class="relative z-0 w-full mb-5">
                                ...
                                >
                                    ...
                                    @foreach(\App\Models\Organization::get() as $organization)
                                        <option @if($post->organization_id === $organization->id) selected @endif value="{{ $organization->id }}">{{ $organization->name }}</option>
                                    @endforeach
                                    {{--
        ...
        <div class="bg-white rounded-3xl border border-red-200 shadow-md dark:bg-gray-800 dark:border-gray-700 md:w-2/3 w-full mx-auto my-8">
            <form action="{{ route('posts.destroy', ['post' => $post->id]) }}" method="post" clas="my-3">
                @csrf
                @method('DELETE')
                <h3 class="border-b border-gray-200 p-3 text-xl">คุณแน่ใจใช่มั้ยที่จะลบ?</h3>
                <div class="p-3">
                    <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                        กรุณาพิมพ์ <span class="font-bold">{{ $post->title }}</span> เพื่อยืนยันการลบ
                    </label>
                ...
                <div class="flex justify-end">
                    <button type="submit" class="bg-rose-500 hover:bg-rose-700 text-white py-2 px-4 rounded-xl mb-4 mx-4 w-full">
                        ลบคำร้องเรียน
                    </button>
                </div>
            </form>
